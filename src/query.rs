extern {
    ...
    fn cache_get(
        arg1 : *const u8, arg2 : u32, arg3 : *mut u32, arg4 : *mut u32
    ) -> *mut u8;
    fn cache_set(
        arg1 : *const u8,
        arg2 : u32,
    ...
    fn case_diffb(
        arg1 : *const u8, arg2 : u32, arg3 : *const u8
    ) -> i32;
    ...
    fn log_nodata(
        arg1 : *const u8, arg2 : *const u8, arg3 : *const u8, arg4 : u32
    );
...
#[no_mangle]
pub unsafe extern fn query_forwardonly() {
...
unsafe extern fn cleanup(mut z : *mut query) {
    let mut j : i32;
    let mut k : i32;
    ...
    'loop3: loop {
        ...
        'loop6: loop {
            ...
            dns_domain_free(
                &mut (*z).ns[j as (usize)][k as (usize)] as (*mut *mut u8)
            );
            k = k + 1;
        }
        j = j + 1;
...
unsafe extern fn globalip(
    mut d : *mut u8, mut ip : *mut u8
) -> i32 {
    if dns_domain_equal(
           d as (*const u8),
...
unsafe extern fn rqa(mut z : *mut query) -> i32 {
...
unsafe extern fn ttlget(mut buf : *mut u8) -> u32 {
    let mut ttl : u32;
...
unsafe extern fn smaller(
    mut buf : *mut u8, mut len : u32, mut pos1 : u32, mut pos2 : u32
) -> i32 {
    ...
    r = byte_diff(header1.as_mut_ptr(),4u32,header2.as_mut_ptr());
    if r < 0i32 {
        1i32
    } else if r > 0i32 {
        0i32
...
unsafe extern fn save_start() {
    save_len = 0u32;
    save_ok = 1u32;
}

unsafe extern fn save_data(mut buf : *const u8, mut len : u32) {
    if save_ok == 0 {
    } else if len as (usize) > ::std::mem::size_of::<[u8; 8192]>(
                               ).wrapping_sub(
                                   save_len as (usize)
                               ) {
        save_ok = 0u32;
...
unsafe extern fn cachegeneric(
    ...
    if len > 255u32 {
        ...
        byte_copy(key.as_mut_ptr(),2u32,type_ as (*mut u8));
        byte_copy(key.as_mut_ptr().offset(2isize),len,d as (*mut u8));
...
unsafe extern fn save_finish(
    mut type_ : *const u8, mut d : *const u8, mut ttl : u32
) { if save_ok == 0 {
    } else {
        cachegeneric(
            type_,
...
unsafe extern fn doit(mut z : *mut query, mut state : i32) -> i32 {
    let mut _currentBlock;
    let mut key : [u8; 257];
    ...
    if state == 1i32 {
        if {
            ...
            whichserver = (*z).dt.servers.offset(
                              4u32.wrapping_mul((*z).dt.curserver) as (isize)
                          );
                            ...
                            if _currentBlock == 348 {
                                ...
                                if flagcname == 0 && (rcode == 0) && (flagout == 0) && (flagreferral != 0) && (flagsoa == 0) {
                                    if dns_domain_equal(
                                        ...
                                        byte_zero(whichserver as (*mut u8),4u32);
                                        _currentBlock = 183;
                                    } else {
                                        _currentBlock = 13;
                                    }
                                ...
                                if _currentBlock == 183 {
                                    ...
                                    k = numanswers as (i32) + numauthority as (i32) + numglue as (i32);
                                    records = alloc(
                                                  (k as (usize)).wrapping_mul(
                                                      ::std::mem::size_of::<u32>()
                                                  ) as (u32)
                                              ) as (*mut u32);
                                    if records.is_null() {
                                        ...
                                        if _currentBlock == 348 {
                                                            ...
                                                            if !(byte_diff(
                                                                ...
                                                                if byte_diff(
                                                                    ...
                                                                    'loop116: loop {
                                                                        ...
                                                                        if datalen as (u32) > len.wrapping_sub(
                                                                                                  pos
                                                                                              ) {
                                                                            _currentBlock = 348;
                                                                            break 'loop21;
                                            ...
                                            if _currentBlock == 348 {
                                                ...
                                                if flagcname != 0 {
                                                    ...
                                                    log_stats();
                                                    if flagout != 0 || flagsoa != 0 || flagreferral == 0 {
                                                        if (*z).level != 0 {
                                                            pos = posanswers;
                                                            j = 0i32;
                                                            'loop78: loop {
    ...
    'loop183: loop {
        if _currentBlock == 183 {
            ...
            if _currentBlock == 185 {
                ...
                if dns_domain_copy(
                       &mut (*z).name[
                                (*z).level.wrapping_add(1u32) as (usize)
                            ] as (*mut *mut u8),
            ...
            if globalip(d,misc.as_mut_ptr()) != 0 {
                if (*z).level != 0 {
                    ...
                    'loop319: loop {
                        ...
                        if byte_diff(
                               (*z).servers[(*z).level.wrapping_sub(1u32) as (usize)].as_mut_ptr(
                               ).offset(
                                   k as (isize)
                               ),
                               4u32,
                ...
                if dlen <= 255u32 {
                    ...
                    byte_copy(key.as_mut_ptr().offset(2isize),dlen,d);
                    ...
                    if !cached.is_null() {
                        ...
                        if !cached.is_null() {
                            ...
                            if typematch((*b"\0\x02\0").as_ptr(),dtype) != 0 {
                                ...
                                if !cached.is_null() && (cachedlen != 0 || byte_diff(
                                                                               dtype as (*mut u8),
                                                                               2u32,
                                                                               (*b"\0\xFF\0").as_ptr(
                                                                               ) as (*mut u8)
                                    ...
                                    if (*z).level != 0 {
                                        ...
                                        'loop253: loop {
                                            ...
                                            'loop255: loop {
                                                ...
                                                if byte_diff(
                                                       (*z).servers[
                                                           (*z).level.wrapping_sub(1u32) as (usize)
                                                       ].as_mut_ptr(
                                                       ).offset(
                                                           k as (isize)
                                                       ),
                            ...
                            if typematch((*b"\0\xFF\0").as_ptr(),dtype) == 0 && (typematch(
                                                                                     (*b"\0\xFC\0").as_ptr(
                                                                                     ),
                                                                                     dtype
                                                                                 ) == 0) && (typematch(
                                ...
                                byte_copy(key.as_mut_ptr(),2u32,dtype as (*mut u8));
                                cached = cache_get(
                                             key.as_mut_ptr() as (*const u8),
                                             dlen.wrapping_add(2u32),
                                             &mut cachedlen as (*mut u32),
                ...
                if _currentBlock == 225 {
                    ...
                    byte_zero((*z).servers[(*z).level as (usize)].as_mut_ptr(),64u32);
                    j = 0i32;
                    'loop226: loop {
                        if !(j < 16i32) {
                            break;
...
#[no_mangle]
pub unsafe extern fn query_start(
    mut z : *mut query,
    mut dn : *mut u8,
    mut type_ : *mut u8,
    mut class : *mut u8,
    ...
    if byte_diff(
        ...
        (if dns_domain_copy(
             ...
             byte_copy((*z).type_.as_mut_ptr(),2u32,type_);
             byte_copy((*z).class.as_mut_ptr(),2u32,class);
             byte_copy((*z).localip.as_mut_ptr(),4u32,localip);
             doit(z,0i32)
         })
    }
}
...
#[no_mangle]
pub unsafe extern fn query_get(
    mut z : *mut query, mut x : *mut pollfd, mut stamp : *mut taia
) -> i32 {
    let switch1
        = dns_transmit_get(
              &mut (*z).dt as (*mut dns_transmit),
...
#[no_mangle]
pub unsafe extern fn query_io(
    mut z : *mut query, mut x : *mut pollfd, mut deadline : *mut taia
) {
