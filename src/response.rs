...
#[no_mangle]
pub unsafe extern fn response_addbytes(
...
#[no_mangle]
pub unsafe extern fn response_addname(mut d : *const u8) -> i32 {
    ...
    'loop1: loop {
        ...
        'loop4: loop {
            ...
            if dns_domain_equal(
                   d,
                   name[i as (usize)].as_mut_ptr() as (*const u8)
               ) != 0 {
                _currentBlock = 13;
                break 'loop1;
            }
        ...
        if dlen <= 128u32 {
            if name_num < 100u32 {
                byte_copy(
                    name[name_num as (usize)].as_mut_ptr(),
                    dlen,
                    d as (*mut u8)
                );
...
#[no_mangle]
pub unsafe extern fn response_query(
    mut q : *const u8, mut qtype : *const u8, mut qclass : *const u8
) -> i32 {
    response_len = 0u32;
    name_num = 0u32;
    if response_addbytes(
           (*b"\0\0\x81\x80\0\x01\0\0\0\0\0\0\0").as_ptr(),
           12u32
       ) == 0 {
        0i32
...
#[no_mangle]
pub unsafe extern fn response_hidettl() {
    flaghidettl = 1i32;
}

#[no_mangle]
pub unsafe extern fn response_rstart(
    mut d : *const u8, mut type_ : *const u8, mut ttl : u32
) -> i32 {
    let mut ttlstr : [u8; 4];
...
#[no_mangle]
pub unsafe extern fn response_rfinish(mut x : i32) {
    uint16_pack_big(
        response.as_mut_ptr().offset(dpos as (isize)).offset(-2isize),
...
#[no_mangle]
pub unsafe extern fn response_cname(
    mut c : *const u8, mut d : *const u8, mut ttl : u32
) -> i32 {
...
#[no_mangle]
pub unsafe extern fn response_nxdomain() {
    let _rhs = 3i32;
...
#[no_mangle]
pub unsafe extern fn response_servfail() {
    let _rhs = 2i32;
    let _lhs = &mut response[3usize];
    *_lhs = (*_lhs as (i32) | _rhs) as (u8);
}
...
#[no_mangle]
pub unsafe extern fn response_id(mut id : *const u8) {
    byte_copy(response.as_mut_ptr(),2u32,id as (*mut u8));
}

#[no_mangle]
pub unsafe extern fn response_tc() {
    let _rhs = 2i32;
    let _lhs = &mut response[2usize];
    *_lhs = (*_lhs as (i32) | _rhs) as (u8);
