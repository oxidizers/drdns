...
#[no_mangle]
pub unsafe extern fn subgetopt(
    ...
    if argv.is_null() || subgetoptind >= argc || (*argv.offset(
                                                       subgetoptind as (isize)
                                                   )).is_null(
                                                 ) {
        subgetoptdone
    } else {
        if subgetoptpos != 0 && (*(*argv.offset(
                                        subgetoptind as (isize)
                                    )).offset(
                                      subgetoptpos as (isize)
                                  ) == 0) {
            subgetoptind = subgetoptind + 1;
            subgetoptpos = 0i32;
            if subgetoptind >= argc || (*argv.offset(
                                             subgetoptind as (isize)
                                         )).is_null(
                                       ) {
        ...
        if subgetoptpos == 0 {
            if *(*argv.offset(subgetoptind as (isize))).offset(
                    0isize
                ) as (i32) != b'-' as (i32) {
                return subgetoptdone;
            } else {
                subgetoptpos = subgetoptpos + 1;
                c = *(*argv.offset(subgetoptind as (isize))).offset(
                         1isize
                     ) as (i32);
        ...
        (if _currentBlock == 8 {
             ...
             if *s.offset(1isize) as (i32) == b':' as (i32) {
                 subgetoptarg = (*argv.offset(subgetoptind as (isize))).offset(
                                    subgetoptpos as (isize)
                                );
                 subgetoptind = subgetoptind + 1;
                 subgetoptpos = 0i32;
