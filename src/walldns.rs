...
#[no_mangle]
pub static mut starting: *const u8 = (*b"starting walldns\n\0").as_ptr();

#[no_mangle]
pub unsafe extern "C" fn initialize() {}

#[no_mangle]
pub unsafe extern "C" fn respond(mut q: *mut u8, mut qtype: *mut u8) -> i32 {
    let mut flaga: i32;
    let mut flagptr: i32;
    let mut ip: [u8; 4];
    let mut j: i32;
    ...
    if flaga != 0 || flagptr != 0 {
        if dd(q as (*const u8), (*b"\0").as_ptr(), ip.as_mut_ptr()) == 4i32 {
            if flaga != 0 {
                if response_rstart(q as (*const u8), (*b"\0\x01\0").as_ptr(), 655360u32) == 0 {
                    return 0i32;
                } else if response_addbytes(ip.as_mut_ptr() as (*const u8), 4u32) == 0 {
                    return 0i32;
                } else {
            ...
            if j >= 0i32 {
                ...
                if flagptr != 0 {
                    if response_rstart(q as (*const u8), (*b"\0\x0C\0").as_ptr(), 655360u32) == 0 {
                        return 0i32;
