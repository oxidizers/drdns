...
#[no_mangle]
pub unsafe extern fn die_usage() {
    strerr_die(
        100i32,
...
#[no_mangle]
pub unsafe extern fn die_generate() {
    strerr_die(
        111i32,
...
#[no_mangle]
pub unsafe extern fn die_parse() {
    strerr_die(
        111i32,
        (*b"axfr-get: fatal: \0").as_ptr(),
...
#[no_mangle]
pub unsafe extern fn x_getname(
    mut buf : *mut u8,
    mut len : u32,
    mut pos : u32,
    mut out : *mut *mut u8
...
#[no_mangle]
pub unsafe extern fn x_skipname(
    mut buf : *mut u8, mut len : u32, mut pos : u32
) -> u32 {
...
#[no_mangle]
pub unsafe extern fn die_write() {
    strerr_die(
        111i32,
        (*b"axfr-get: fatal: \0").as_ptr(),
...
pub static mut netread
    ...
    = buffer {
          ...
          fd: 6i32,
          op: saferead as (unsafe extern fn() -> i32)
      };
...
#[no_mangle]
pub unsafe extern fn netget(mut buf : *mut u8, mut len : u32) {
    let mut r : i32;
    'loop1: loop {
        if !(len > 0u32) {
            break;
...
#[no_mangle]
pub unsafe extern fn put(mut buf : *mut u8, mut len : u32) {
    if buffer_put(
           &mut b as (*mut buffer),
           buf as (*const u8),
           len
...
#[no_mangle]
pub unsafe extern fn printable(mut ch : u8) -> i32 {
