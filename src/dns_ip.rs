...
#[no_mangle]
pub unsafe extern fn dns_ip4_packet(
    mut out : *mut stralloc, mut buf : *const u8, mut len : u32
) -> i32 {
                      ...
                      if byte_diff(
                          ...
                          if byte_diff(
                              ...
                              if datalen as (i32) == 4i32 {
                                  if dns_packet_copy(buf,len,pos,header.as_mut_ptr(),4u32) == 0 {
                                      _currentBlock = 15;
                                      break;
                                  }
                                  if stralloc_catb(
...
#[no_mangle]
pub unsafe extern fn dns_ip4(
    mut out : *mut stralloc, mut fqdn : *const stralloc
) -> i32 {
    let mut _currentBlock;
    let mut i : u32;
    let mut code : u8;
        ...
        'loop2: loop {
            ...
            if !(ch as (i32) == b'[' as (i32) || ch as (i32) == b']' as (i32)) {
                if ch as (i32) == b'.' as (i32) {
                    if stralloc_append(
                           out,
                           &mut code as (*mut u8) as (*const u8)
                       ) == 0 {
                        _currentBlock = 20;
                        break;
                    }
