...
unsafe extern fn allwrite(
    mut op : unsafe extern fn(i32, *const u8, u32) -> i32,
    mut fd : i32,
    mut buf : *const u8,
    mut len : u32
) -> i32 {
...
#[no_mangle]
pub unsafe extern fn buffer_flush(mut s : *mut buffer) -> i32 {
    let mut p : i32;
    p = (*s).p as (i32);
    if p == 0 {
        0i32
        ...
        (*s).p = 0u32;
        allwrite(
            (*s).op as (unsafe extern fn(i32, *const u8, u32) -> i32),
            (*s).fd,
            (*s).x as (*const u8),
            p as (u32)
        )
...
#[no_mangle]
pub unsafe extern fn buffer_putalign(
    mut s : *mut buffer, mut buf : *const u8, mut len : u32
) -> i32 {
    let mut _currentBlock;
    let mut n : u32;
    'loop1: loop {
...
#[no_mangle]
pub unsafe extern fn buffer_put(
    mut s : *mut buffer, mut buf : *const u8, mut len : u32
) -> i32 {
    let mut _currentBlock;
    let mut n : u32;
...
#[no_mangle]
pub unsafe extern fn buffer_putflush(
    mut s : *mut buffer, mut buf : *const u8, mut len : u32
) -> i32 {
    if buffer_flush(s) == -1i32 {
        -1i32
...
#[no_mangle]
pub unsafe extern fn buffer_putsalign(
    mut s : *mut buffer, mut buf : *const u8
) -> i32 {
    buffer_putalign(s,buf,str_len(buf))
}

#[no_mangle]
pub unsafe extern fn buffer_puts(
    mut s : *mut buffer, mut buf : *const u8
) -> i32 {
...
#[no_mangle]
pub unsafe extern fn buffer_putsflush(
    mut s : *mut buffer, mut buf : *const u8
) -> i32 {
