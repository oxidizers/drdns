...
#[no_mangle]
pub unsafe extern fn address_diff(
    mut p : *mut address, mut q : *mut address
) -> i32 {
    let mut r : i32;
    r = byte_diff(
            (*p).location.as_mut_ptr(),
            2u32,
            (*q).location.as_mut_ptr()
        );
    if r < 0i32 {
        -1i32
    } else if r > 0i32 {
...
#[no_mangle]
pub unsafe extern fn address_sort(mut z : *mut address, mut n : u32) {
    let mut i : u32;
    let mut j : u32;
    let mut p : u32;
    let mut q : u32;
...
static mut f
    : [stralloc; 3]
    = [stralloc { s: 0 as (*mut u8), len: 0u32, a: 0u32 }; 3];
...
pub unsafe extern fn _c_main() -> i32 {
    ...
    'loop10: loop {
        ...
        'loop21: loop {
            ...
            if address_diff(
                   x.s.offset(i as (isize)),
                   x.s.offset(j as (isize))
               ) != 0 {
                break;
            }
            j = j + 1;
