...
#[no_mangle]
pub unsafe extern fn cdb_make_start(
    mut c : *mut cdb_make, mut fd : i32
) -> i32 {
...
unsafe extern fn posplus(
    mut c : *mut cdb_make, mut len : u32
) -> i32 {
...
#[no_mangle]
pub unsafe extern fn cdb_make_addend(
    mut c : *mut cdb_make,
    mut keylen : u32,
    mut datalen : u32,
    mut h : u32
    ...
    if head.is_null() || (*head).num >= 1000i32 {
        head = alloc(
                   ::std::mem::size_of::<cdb_hplist>() as (u32)
               ) as (*mut cdb_hplist);
        if head.is_null() {
            return -1i32;
        } else {
...
#[no_mangle]
pub unsafe extern fn cdb_make_addbegin(
    mut c : *mut cdb_make, mut keylen : u32, mut datalen : u32
) -> i32 {
    let mut buf : [u8; 8];
...
#[no_mangle]
pub unsafe extern fn cdb_make_add(
    mut c : *mut cdb_make,
    mut key : *const u8,
    mut keylen : u32,
    mut data : *const u8,
...
#[no_mangle]
pub unsafe extern fn cdb_make_finish(mut c : *mut cdb_make) -> i32 {
    ...
    'loop3: loop {
        ...
        'loop51: loop {
            ...
            let _rhs = 1;
            let _lhs
                = &mut (*c).count[(255u32 & (*x).hp[i as (usize)].h) as (usize)];
    ...
    if memsize > u {
        ...
        (if (*c).split.is_null() {
                     ...
                     *(*c).split.offset(
                          {
                              let _rhs = 1;
                              let _lhs
                                  = &mut (*c).start[(255u32 & (*x).hp[i as (usize)].h) as (usize)];
             ...
             'loop13: loop {
                 ...
                 uint32_pack(
                     (*c).final_.as_mut_ptr().offset((8i32 * i) as (isize)).offset(
                         4isize
                     ),
                     len
                 );
