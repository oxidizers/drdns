...
pub unsafe extern "C" fn usage() {
    strerr_die(
        100i32,
        (*b"rbldns-conf: usage: rbldns-conf acct logacct /rbldns myip base\0").as_ptr(),
        0i32 as (*const u8),
        0i32 as (*const u8),
        0i32 as (*const u8),
        0i32 as (*const u8),
...
#[no_mangle]
pub static mut base: *mut u8 = 0 as (*mut u8);
...
pub unsafe extern "C" fn _c_main(mut argc: i32, mut argv: *mut *mut u8) -> i32 {
    ...
    init(dir as (*const u8), (*b"rbldns-conf: fatal: \0").as_ptr());
    makelog(
        loguser as (*const u8),
        (*pw).pw_uid as (i32),
        (*pw).pw_gid as (i32),
    ...
    finish();
    perm(0o644i32);
    start((*b"env/BASE\0").as_ptr());
    outs(base as (*const u8));
    outs((*b"\n\0").as_ptr());
    finish();
    perm(0o644i32);
