    ...
    ) == -1i32
    {
        ...
        (if _currentBlock == 3 {
            ...
            ) != 0
            {
                 ...
                 buffer_puts(buffer_1, (*b"ALERT:took more than 1 second\n\0").as_ptr());
             }
             0i32
         } else {
             -1i32
...
pub unsafe extern "C" fn qt_add(
    ...
    if *q == 0 {
        ...
        'loop2: loop {
            ...
            if dns_domain_equal((*qt.s.offset(i as (isize))).owner as (*const u8), q) != 0 {
                if dns_domain_equal(
...
pub unsafe extern "C" fn query_add(mut owner: *const u8, mut type_: *const u8) {
    ...
    if _currentBlock == 2 {
        ...
        'loop7: loop {
            ...
            if dns_domain_suffix(owner, (*ns.s.offset(i as (isize))).owner as (*const u8)) != 0 {
                j = 0i32;
                'loop11: loop {
...
pub unsafe extern "C" fn ns_add(mut owner: *const u8, mut server: *const u8) {
    ...
    'loop1: loop {
        ...
        if dns_domain_equal((*ns.s.offset(i as (isize))).owner as (*const u8), owner) != 0 {
            if dns_domain_equal((*ns.s.offset(i as (isize))).ns as (*const u8), server) != 0 {
                _currentBlock = 21;
                break;
            }
        }
    ...
    if _currentBlock == 2 {
        ...
        'loop9: loop {
            ...
            if dns_domain_suffix((*query.s.offset(i as (isize))).owner as (*const u8), owner) != 0 {
                j = 0i32;
                'loop13: loop {
                    if !(j as (u32) < address.len) {
                        break;
...
pub unsafe extern "C" fn address_add(mut owner: *const u8, mut ip: *const u8) {
    ...
    if _currentBlock == 2 {
        ...
        'loop7: loop {
            ...
            if dns_domain_equal((*ns.s.offset(i as (isize))).ns as (*const u8), owner) != 0 {
                j = 0i32;
                'loop11: loop {
