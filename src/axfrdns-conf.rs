...
pub unsafe extern fn usage() {
    strerr_die(
        100i32,
        (*b"axfrdns-conf: usage: axfrdns-conf acct logacct /axfrdns /tinydns myip\0").as_ptr(
        ),
        0i32 as (*const u8),
        0i32 as (*const u8),
        0i32 as (*const u8),
...
#[no_mangle]
pub static mut tinydns : *mut u8 = 0 as (*mut u8);
...
pub unsafe extern fn _c_main(
    ...
    tinydns = *argv.offset(4isize);
    if tinydns.is_null() {
        usage();
    }
    if *tinydns.offset(0isize) as (i32) != b'/' as (i32) {
        usage();
    }
    myip = *argv.offset(5isize);
    if myip.is_null() {
        usage();
    }
    ...
    init(dir as (*const u8),(*b"axfrdns-conf: fatal: \0").as_ptr());
    makelog(
        loguser as (*const u8),
        (*pw).pw_uid as (i32),
        (*pw).pw_gid as (i32)
    ...
    outs(
        (*b" softlimit -d300000 tcpserver -vDRHl0 -x tcp.cdb -- \"$IP\" 53 \0").as_ptr(
        )
    );
    outs(auto_home);
    outs((*b"/bin/axfrdns\n\'\n\0").as_ptr());
    finish();
    perm(0o755i32);
    start((*b"Makefile\0").as_ptr());
    outs((*b"tcp.cdb: tcp\n\0").as_ptr());
    outs((*b"\ttcprules tcp.cdb tcp.tmp < tcp\n\0").as_ptr());
    finish();
    perm(0o644i32);
    start((*b"tcp\0").as_ptr());
    outs(
        (*b"# sample line:  1.2.3.4:allow,AXFR=\"heaven.af.mil/3.2.1.in-addr.arpa\"\n\0").as_ptr(
        )
    );
    outs((*b":deny\n\0").as_ptr());
    finish();
