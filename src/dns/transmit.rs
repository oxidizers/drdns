...
impl DnsTransmit {
    ...
    pub unsafe fn io(d: *mut DnsTransmit, x: *mut libc::pollfd, deadline: *mut TaiA) {
        (*x).fd = (*d).s1 - 1i32;
    ...
    pub unsafe fn get(d: *mut DnsTransmit, x: *const libc::pollfd, when: *const TaiA) -> i32 {
        ...
        if (*x).revents == 0 {
            ...
            {
                 0i32
             } else if DnsTransmit::serverwantstcp(
                udpbuf.as_mut_ptr() as (*const u8),
                r as (u32),
            ) != 0
            {
                 DnsTransmit::firsttcp(d)
             } else if DnsTransmit::serverfailed(
                udpbuf.as_mut_ptr() as (*const u8),
                r as (u32),
            ) != 0
            {
            ...
            r = libc::read(fd, &mut ch as (*mut u8) as (*mut libc::c_void), 1usize) as (i32);
            (if r <= 0i32 {
                 DnsTransmit::nexttcp(d)
             } else {
        ...
        'loop1: loop {
            ...
            ) != 0
            {
                ...
                *(*d).query.offset(3isize) = random::random(256u32) as (u8);
                ...
                if errno::errno() == Errno(libc::EINPROGRESS) ||
                    errno::errno() == Errno(libc::EWOULDBLOCK)
                {
                    current_block = 10;
                    break;
                }
