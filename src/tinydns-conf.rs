...
pub unsafe extern fn usage() {
    strerr_die(
        100i32,
        (*b"tinydns-conf: usage: tinydns-conf acct logacct /tinydns myip\0").as_ptr(
        ),
        0i32 as (*const u8),
        0i32 as (*const u8),
        0i32 as (*const u8),
...
pub unsafe extern fn _c_main(
    ...
    init(dir as (*const u8),(*b"tinydns-conf: fatal: \0").as_ptr());
    makelog(
        loguser as (*const u8),
        (*pw).pw_uid as (i32),
        (*pw).pw_gid as (i32)
    ...
    outs((*b" envdir ./env softlimit -d300000 \0").as_ptr());
    outs(auto_home);
    outs((*b"/bin/tinydns\n\0").as_ptr());
    finish();
    perm(0o755i32);
    ...
    finish();
    perm(0o644i32);
    start((*b"root/add-ns\0").as_ptr());
    outs((*b"#!/bin/sh\nexec \0").as_ptr());
    outs(auto_home);
    outs(
        (*b"/bin/tinydns-edit data data.new add ns ${1+\"$@\"}\n\0").as_ptr(
        )
    );
    finish();
    perm(0o755i32);
    start((*b"root/add-childns\0").as_ptr());
    outs((*b"#!/bin/sh\nexec \0").as_ptr());
    outs(auto_home);
    outs(
        (*b"/bin/tinydns-edit data data.new add childns ${1+\"$@\"}\n\0").as_ptr(
        )
    );
    finish();
    perm(0o755i32);
    start((*b"root/add-host\0").as_ptr());
    outs((*b"#!/bin/sh\nexec \0").as_ptr());
    outs(auto_home);
    outs(
        (*b"/bin/tinydns-edit data data.new add host ${1+\"$@\"}\n\0").as_ptr(
        )
    );
    finish();
    perm(0o755i32);
    start((*b"root/add-alias\0").as_ptr());
    outs((*b"#!/bin/sh\nexec \0").as_ptr());
    outs(auto_home);
    outs(
        (*b"/bin/tinydns-edit data data.new add alias ${1+\"$@\"}\n\0").as_ptr(
        )
    );
    finish();
    perm(0o755i32);
    start((*b"root/add-mx\0").as_ptr());
    outs((*b"#!/bin/sh\nexec \0").as_ptr());
    outs(auto_home);
    outs(
        (*b"/bin/tinydns-edit data data.new add mx ${1+\"$@\"}\n\0").as_ptr(
        )
    );
    finish();
    perm(0o755i32);
