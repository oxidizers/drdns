extern {
    ...
    fn socket_connect4(
        arg1 : i32, arg2 : *const u8, arg3 : u16
    ) -> i32;
...
unsafe extern fn queryfree(mut d : *mut dns_transmit) {
    if (*d).query.is_null() {
    } else {
        alloc_free((*d).query);
...
unsafe extern fn socketfree(mut d : *mut dns_transmit) {
    if (*d).s1 == 0 {
    } else {
        close((*d).s1 - 1i32);
        (*d).s1 = 0i32;
...
unsafe extern fn packetfree(mut d : *mut dns_transmit) {
    if (*d).packet.is_null() {
    } else {
        alloc_free((*d).packet);
...
#[no_mangle]
pub unsafe extern fn dns_transmit_free(mut d : *mut dns_transmit) {
    queryfree(d);
    socketfree(d);
    packetfree(d);
}
...
unsafe extern fn randombind(mut d : *mut dns_transmit) -> i32 {
...
unsafe extern fn thistcp(mut d : *mut dns_transmit) -> i32 {
    let mut _currentBlock;
    let mut now : taia;
    let mut ip : *const u8;
    socketfree(d);
...
unsafe extern fn firsttcp(mut d : *mut dns_transmit) -> i32 {
    (*d).curserver = 0u32;
    thistcp(d)
}

unsafe extern fn thisudp(mut d : *mut dns_transmit) -> i32 {
    let mut _currentBlock;
    let mut ip : *const u8;
    socketfree(d);
    'loop1: loop {
                ...
                *(*d).query.offset(3isize) = dns_random(256u32) as (u8);
                ...
                if socket_connect4((*d).s1 - 1i32,ip,53u16) == 0i32 {
                    if send(
                           ...
                           0i32
                       ) as (usize) == (*d).querylen.wrapping_sub(2u32) as (usize) {
                        _currentBlock = 12;
                        break 'loop1;
                    }
                }
...
unsafe extern fn firstudp(mut d : *mut dns_transmit) -> i32 {
    (*d).curserver = 0u32;
    thisudp(d)
}

#[no_mangle]
pub unsafe extern fn dns_transmit_start(
...
#[no_mangle]
pub unsafe extern fn dns_transmit_io(
    mut d : *mut dns_transmit,
    mut x : *mut pollfd,
...
unsafe extern fn nextudp(mut d : *mut dns_transmit) -> i32 {
    (*d).curserver = (*d).curserver.wrapping_add(1u32);
    thisudp(d)
}

unsafe extern fn nexttcp(mut d : *mut dns_transmit) -> i32 {
    (*d).curserver = (*d).curserver.wrapping_add(1u32);
    thistcp(d)
}

unsafe extern fn irrelevant(
    mut d : *const dns_transmit, mut buf : *const u8, mut len : u32
) -> i32 {
    let mut out : [u8; 12];
    let mut dn : *mut u8;
    let mut pos : u32;
...
unsafe extern fn serverwantstcp(
    mut buf : *const u8, mut len : u32
) -> i32 {
    let mut out : [u8; 12];
...
#[no_mangle]
pub unsafe extern fn dns_transmit_get(
    mut d : *mut dns_transmit,
    mut x : *const pollfd,
    mut when : *const taia
) -> i32 {
    ...
    if (*x).revents == 0 {
        (if taia_less(
             ...
             errno = error_timeout;
             (if (*d).tcpstate == 0i32 { nextudp(d) } else { nexttcp(d) })
         })
        ...
        (if r <= 0i32 {
             ...
             (if (*d).udploop == 2u32 { 0i32 } else { nextudp(d) })
         } else {
             socketfree(d);
