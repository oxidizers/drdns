...
#[no_mangle]
pub unsafe extern fn ttdparse(
    mut sa : *mut stralloc, mut ttd : *mut u8
) {
    let mut i : u32;
    let mut ch : u8;
    byte_zero(ttd,8u32);
...
#[no_mangle]
pub unsafe extern fn locparse(
    mut sa : *mut stralloc, mut loc : *mut u8
) {
...
#[no_mangle]
pub unsafe extern fn ipprefix_cat(
    mut out : *mut stralloc, mut s : *mut u8
) {
    let mut u : usize;
    let mut ch : u8;
    let mut j : u32;
...
#[no_mangle]
pub unsafe extern fn txtparse(mut sa : *mut stralloc) {
    let mut ch : u8;
    let mut i : u32;
    let mut j : u32;
    j = 0u32;
    ...
    'loop1: loop {
        ...
        if ch as (i32) == b'\\' as (i32) {
            ...
            if ch as (i32) >= b'0' as (i32) && (ch as (i32) <= b'7' as (i32)) {
                ch = (ch as (i32) - b'0' as (i32)) as (u8);
                if i < (*sa).len && (*(*sa).s.offset(
                                          i as (isize)
                                      ) as (i32) >= b'0' as (i32)) && (*(*sa).s.offset(
                                                                            i as (isize)
                                                                        ) as (i32) <= b'7' as (i32)) {
                    ch = (ch as (i32) << 3i32) as (u8);
                    ch = (ch as (i32) + (*(*sa).s.offset(
                                              {
                                                  let _old = i;
                                                  i = i.wrapping_add(1u32);
                                                  _old
                                              } as (isize)
                                          ) as (i32) - b'0' as (i32))) as (u8);
                    if i < (*sa).len && (*(*sa).s.offset(
                                              i as (isize)
                                          ) as (i32) >= b'0' as (i32)) && (*(*sa).s.offset(
                                                                                i as (isize)
...
#[no_mangle]
pub unsafe extern fn defaultsoa_init(mut fd : i32) {
    let mut st : stat;
...
#[no_mangle]
pub unsafe extern fn rr_add(mut buf : *const u8, mut len : u32) {
...
#[no_mangle]
pub unsafe extern fn rr_addname(mut d : *const u8) {
    rr_add(d,dns_domain_length(d));
}

#[no_mangle]
pub unsafe extern fn rr_start(
    mut type_ : *const u8,
    mut ttl : usize,
    mut ttd : *const u8,
    mut loc : *const u8
...
#[no_mangle]
pub unsafe extern fn rr_finish(mut owner : *const u8) {
    if byte_diff(
           owner as (*mut u8),
           2u32,
...
static mut f
    : [stralloc; 15]
    = [stralloc { s: 0 as (*mut u8), len: 0u32, a: 0u32 }; 15];
...
pub unsafe extern fn _c_main() -> i32 {
    ...
    'loop6: loop {
        ...
        if switch1 as (i32) == b':' as (i32) {
            ...
            if stralloc_append(
                   &mut f[3usize] as (*mut stralloc),
                   (*b"\0").as_ptr()
               ) == 0 {
                nomem();
            }
            if scan_ulong(
                   f[3usize].s as (*const u8),
                   &mut ttl as (*mut usize)
               ) == 0 {
                ttl = 86400usize;
            }
            ...
            if stralloc_append(
                   &mut f[4usize] as (*mut stralloc),
                   (*b"\0").as_ptr()
               ) == 0 {
                nomem();
            }
            if scan_ulong(
                   f[4usize].s as (*const u8),
                   &mut ttl as (*mut usize)
               ) == 0 {
                ttl = 86400usize;
            }
            ...
            if byte_chr(f[2usize].s,f[2usize].len,b'.' as (i32)) >= f[  2usize
                                                                    ].len {
                if stralloc_cats(
                       &mut f[2usize] as (*mut stralloc),
                       (*b".mx.\0").as_ptr()
            ...
            if stralloc_append(
                   &mut f[5usize] as (*mut stralloc),
                   (*b"\0").as_ptr()
               ) == 0 {
                nomem();
            }
            if scan_ulong(
                   f[5usize].s as (*const u8),
                   &mut u as (*mut usize)
               ) == 0 {
                uint32_unpack_big(
            ...
            if stralloc_append(
                   &mut f[6usize] as (*mut stralloc),
                   (*b"\0").as_ptr()
               ) == 0 {
                nomem();
            }
            if scan_ulong(
                   f[6usize].s as (*const u8),
                   &mut u as (*mut usize)
               ) == 0 {
                uint32_unpack_big(
            ...
            if stralloc_append(
                   &mut f[7usize] as (*mut stralloc),
                   (*b"\0").as_ptr()
               ) == 0 {
                nomem();
            }
            if scan_ulong(
                   f[7usize].s as (*const u8),
                   &mut u as (*mut usize)
               ) == 0 {
                uint32_unpack_big(
            ...
            if stralloc_append(
                   &mut f[8usize] as (*mut stralloc),
                   (*b"\0").as_ptr()
               ) == 0 {
                nomem();
            }
            if scan_ulong(
                   f[8usize].s as (*const u8),
                   &mut ttl as (*mut usize)
               ) == 0 {
                ttl = 2560usize;
            }
