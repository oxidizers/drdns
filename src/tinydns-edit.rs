...
static mut f
    : [stralloc; 10]
    = [stralloc { s: 0 as (*mut u8), len: 0u32, a: 0u32 }; 10];
...
#[no_mangle]
pub unsafe extern fn put(mut buf : *const u8, mut len : u32) {
...
pub unsafe extern fn _c_main(
    ...
    if fchmod(
           fdnew,
           (st.st_mode as (i32) & 0o644i32) as (u16)
       ) == -1i32 {
        die_write();
    }
    buffer_init(
    ...
    if mode as (i32) == b'@' as (i32) || mode as (i32) == b'&' as (i32) || mode as (i32) == b'.' as (i32) {
        i = 0i32;
        'loop68: loop {
            if !(i < 26i32) {
                break;
