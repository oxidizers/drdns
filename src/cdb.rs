...
#[no_mangle]
pub unsafe extern fn cdb_free(mut c : *mut cdb) {
    if !(*c).map.is_null() {
        munmap(
...
#[no_mangle]
pub unsafe extern fn cdb_init(mut c : *mut cdb, mut fd : i32) {
    let mut st : stat;
    let mut x : *mut u8;
    cdb_free(c);
    cdb_findstart(c);
...
#[no_mangle]
pub unsafe extern fn cdb_read(
    mut c : *mut cdb, mut buf : *mut u8, mut len : u32, mut pos : u32
) -> i32 {
...
unsafe extern fn match_(
    mut c : *mut cdb, mut key : *const u8, mut len : u32, mut pos : u32
) -> i32 {
    let mut _currentBlock;
    let mut buf : [u8; 32];
    let mut n : i32;
...
#[no_mangle]
pub unsafe extern fn cdb_findnext(
    mut c : *mut cdb, mut key : *const u8, mut len : u32
) -> i32 {
    let mut _currentBlock;
    let mut buf : [u8; 8];
    let mut pos : u32;
